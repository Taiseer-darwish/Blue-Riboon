...
const schema = (checkEntityExists) =>
  yup.object().shape({
    entityName: yup
      .string()
    ...
    entityImage: yup
      ...
      .test("file-selected", "Image is required", (value) => value && value[0] instanceof File),
  });
...
function EntityForm({
  ...
  const {
    ...
    resolver: yupResolver(schema(checkEntityExists)),
  });
  ...
  return (
    <form className="w-1/2 flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div>
        ...
        />
        {errors.entityImage && (
          <p className="text-red-500 text-sm mt-1">{errors.entityImage.message}</p>
        )}
      </div>
      <button
        type="submit"
